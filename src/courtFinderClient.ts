    ...
    private performRequest (uri: string): Promise<CourtFinderResponse> {
        return this.request.get({
            ...
            if (response.statusCode !== 200) {
                return new CourtFinderResponse(response.statusCode, false, JSON.parse(response.body).error)
            }
            ...
            courtFinderResponse.addAll(
                responseBody.map((court: any) => {
                    ...
                    if (newCourt.name === 'Birmingham Civil and Family Justice Centre'){
                        newCourt.areasOfLaw.push(AreaOfLaw.MONEY_CLAIMS)
                    }
                    return newCourt
                })
            )
